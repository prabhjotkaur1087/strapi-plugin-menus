...
const itemSchema = yup.object().shape({
  order: yup.number(translatedErrors.number).required(translatedErrors.required),
  title: yup.string(translatedErrors.string).required(translatedErrors.required),
  url: yup
    .string(translatedErrors.string)
    .test(
      'is-url',
      getTrad('error.url.invalid'),
      (value) =>
        !!(
          !value ||
